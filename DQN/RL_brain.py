...
class DeepQNetwork:
    def __init__(
            ...
            #n_features表示观测到的特征，比如在平衡车中，可以是车的未知，棒的角度这些特征组成的向量，当然我们知道这些是连续的值，但是我们训练的时候就相当于抽样，这样最后训练完成了即使遇到没有的情况也能做好相应的动作。
        ...
        #n_features表示观测到的特征，比如在平衡车中，可以是车的未知，棒的角度这些特征组成的向量。用这个向量来表示一个状态s。所以下面乘以2.当然我们知道这些是连续的值，但是我们训练的时候就相当于抽样，这样最后训练完成了即使遇到没有的情况也能做好相应的动作。加2是因为还要保存动作，当然在平衡车中只有两个动作，左和右。我们可以用0和1表示，所以占用一个位置即可。reward也是一个标量。
